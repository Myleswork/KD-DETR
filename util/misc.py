...
def collate_fn(batch):
    ...
    if 'attn_masks' in batch[1][0]:
        mask_list = []
        for tgt in batch[1]:
            mask_list.append(tgt.pop('attn_masks'))
        batch[0] = nested_tensor_from_tensor_list(batch[0], mask_list)
    else:
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], mask_list=None):
    ...
    if tensor_list[0].ndim == 3:
        ...
        if mask_list is not None:
            for img, msk, pad_img, m in zip(tensor_list, mask_list, tensor, mask):
                pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
                m[: msk.shape[1], :msk.shape[2]].copy_(msk[0])
        else:
...
def log_metrics(ckpt_dir, name, step, value, timestamp=None):
    ...
    url = env.MONGODB_URL
    if url is None:
        print('mongodb url is not set, skip recording...')
        return
    db_name = 'sacred'
    collection_prefix = ''
    ...
    try:
        ...
        update = {"$push": push}
        result = metrics_collection.update_one(query, update, upsert=True)
        if result.upserted_id is not None:
            run_entry = runs_collection.find_one({"_id": _id})
            if run_entry is None:
                raise RuntimeError("Couldn't find run with " "_id='{}'".format(_id))
            info = run_entry['info']
            # This is the first time we are storing this metric
            info.setdefault("metrics", []).append(
                {"name": name, "id": str(result.upserted_id)}
            )
            run_entry["info"] = flatten(info)
            runs_collection.update_one(
                {"_id": run_entry["_id"]}, {"$set": run_entry}
            )
